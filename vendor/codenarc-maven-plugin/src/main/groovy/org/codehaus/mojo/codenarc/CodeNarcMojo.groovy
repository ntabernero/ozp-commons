...
"""
	...
	def items = [
        ...
        if (!log4jConfigFile) {
            def tempLogFile = new File( "${project.getBuild().getDirectory()}/log4j.properties"  )
        ...
        ant.java(classname: "org.codenarc.CodeNarc", classpathref: "codenarc.classpath",fork: "true", failonerror: "false", clonevm: "false", maxmemory: "${maxHeap}m") {
             ...
             * BEGIN OZP PATCH
             *
             * For Windows bug where codenarc-maven-plugin uses the maven pom's name, but if 
             * there are spaces in it the plugin fails.
             *
             * PREVIOUS CODE:
