...
define(['models/DashboardInstanceModel', 'collections/DashboardInstancesCollection'], function(DashboardInstanceModel, DashboardInstancesCollection) {
    describe('DashboardInstancesCollectionSpec', function() {
    
        beforeEach(function(done) {
            this.collection = new DashboardInstancesCollection();
            this.dashboard1 = new DashboardInstanceModel({name: 'test dashboard 1', description: 'A sample dashboard', id: '11111', person: 1})
            this.dashboard2 = new DashboardInstanceModel({name: 'test dashboard 2', description: 'Another sample dashboard', id: '22222', person: 1})
        ...
        it('generates a base url for an empty collection.', function () {
            expect(this.collection.url).to.eql('/ozp/rest/owf/dashboard-instances');
        });
        ...
        it('generates a url for model in collection with no id.', function () {
            ...
            expect(this.collection.at(0).url()).to.eql('/ozp/rest/owf/dashboard-instances');
        });
        
        it('generates a url for model in collection with an id.', function () {
            this.collection.add({name: 'test dashboard', id: '12345', person: 1})
            expect(this.collection.at(0).get('name')).to.eql('test dashboard');
            expect(this.collection.at(0).url()).to.eql('/ozp/rest/owf/dashboard-instances/12345');
        });
        ...
        it("creates a PUT request to the correct URL for a model update.", function() {
            ...
            expect(this.server.requests.length).to.eql(1);
            expect(this.server.requests[0].method).to.eql("PUT");
            expect(this.server.requests[0].url).to.eql("/ozp/rest/owf/dashboard-instances/11111");
